...
Eflt32 CommunicationMinValue(Eflt32 Value)
{
  ...
  MPI_Arg Count = 1;
  MPI_Allreduce(&Value, &ReturnValue, Count, DataType, MPI_MIN, EnzoTopComm);
  END_TIMING;
...
Eflt32 CommunicationMaxValue(Eflt32 Value)
{
  ...
  START_TIMING;
  MPI_Allreduce(&Value, &ReturnValue, Count, DataType, MPI_MAX, EnzoTopComm);
  END_TIMING;
...
int CommunicationSumValues(Eflt32 *Values, int Number)
{
  ...
  START_TIMING;
  MPI_Reduce(buffer, Values, Count, DataType, MPI_SUM, ROOT_PROCESSOR, EnzoTopComm);
  END_TIMING;
...
int CommunicationAllSumValues(Eflt32 *Values, int Number)
{
  ...
  START_TIMING;
  MPI_Allreduce(buffer, Values, Count, DataType, MPI_SUM, EnzoTopComm);
  END_TIMING;
