  ...
  for (gr = D.NgroupsAll-1; gr >= 0; ) {
	...
	// task != 0
...
#ifdef USE_MPI
	  MPI_Recv(&nsubs, 1, MPI_INT, task, task, EnzoTopComm, &status);
	  if (nsubs > 0) {
	    ...
	    MPI_Recv(bufsublen, nsubs, IntDataType, task, task, EnzoTopComm,
		     &status);
	    MPI_Recv(bufsuboffset, nsubs, IntDataType, task, task, EnzoTopComm,
		     &status);
	  ...
	  MPI_Recv(partbuf, len*sizeof(FOF_particle_data),
		   MPI_BYTE, task, task, EnzoTopComm, &status);
...
#ifdef USE_MPI
	if (MyProcessorNumber == task) {
	  MPI_Ssend(&nsubs, 1, MPI_INT, 0, MyProcessorNumber, EnzoTopComm);
	  if(nsubs) {
	  ...
	  MPI_Ssend(Pbuf, D.GroupDatAll[gr-task].Len*sizeof(FOF_particle_data), 
		    MPI_BYTE, 0, MyProcessorNumber, EnzoTopComm);
	} // ENDIF proc == task
