...
//   HYPRE_StructVectorPrint("x.vec",solvec,0);
  ...
  HYPRE_StructGMRESCreate(EnzoTopComm, &solver);
  ...
  if (dtfac > 0.0) { 
    ...
    if (Nglobal == LocDims[0]*LocDims[1]*LocDims[2])  glob_est = loc_est;
      ...
      if (dtnorm > 0.0) 
	MPI_Allreduce(&loc_est,&glob_est,1,DataType,MPI_SUM,EnzoTopComm);
      else
	MPI_Allreduce(&loc_est,&glob_est,1,DataType,MPI_MAX,EnzoTopComm);
    }
#else
    glob_est = loc_est;
#endif
