...
int grid::AccreteOntoAccretingParticle(ActiveParticleType_AccretingParticle* ThisParticle, FLOAT AccretionRadius, 
  ...
  // See Krumholz et al. 2004 Equation 11
  ...
  for (i = 0; i < GridDimension[0]; i++) {
	...
	if ((AccretionRadius*AccretionRadius) > radius2) {
	  ...
	  CellSubtractedMass =  this->dtFixed * ThisParticle->AccretionRate * Weight;
	  if (CellSubtractedMass > 0.25*CellMass) 
	    CellSubtractedMass = 0.25*CellMass;
	  SubtractedMass += CellSubtractedMass
	  BaryonField[DensNum] -= CellSubtracedMass/CellVolume;
	  
	  /* Modify momentum.  Find components of velocity transverse and
	     perpendicular to teh position of the sink.  Increase the transverse
	     velocity such that the angular momentum of the gas is unchanged and
	     leave the radial velocity unchanged. */
	     
	  // First find distance to the cell.
	  ...
	  // Compute the radial component of the momentum density
	  rdotv = 
