		       ...
		       int NBx1R, int NBx2L, int NBx2R)
{
  ...
  if ((NBx0L == MPI_PROC_NULL) && (NBx0R == MPI_PROC_NULL) &&
    ...
    MPI_Allreduce(&Nlocal, &Nglobal, one, DataType, MPI_SUM, EnzoTopComm);  
  }
#else
  Nglobal = Nlocal;
#endif
...
float EnzoVector::dot(EnzoVector *x) const
{
  ...
  if (Nglobal == Nx0*Nx1*Nx2*Nspecies) 
    ...
    MPI_Arg one = 1;
    MPI_Allreduce(&sum, &gsum, one, DataType, MPI_SUM, EnzoTopComm);
  }
#else
  gsum = sum;
#endif
...
float EnzoVector::infnorm() const
{
  ...
  if (Nglobal == Nx0*Nx1*Nx2*Nspecies) 
    ...
    MPI_Datatype DataType = (sizeof(float) == 4) ? MPI_FLOAT : MPI_DOUBLE;
    MPI_Arg one = 1;
    MPI_Allreduce(&max, &gmax, one, DataType, MPI_MAX, EnzoTopComm);
  }
#else
  gmax = max;
#endif
...
float EnzoVector::minval() const
{
  ...
  if (Nglobal == Nx0*Nx1*Nx2*Nspecies) 
    ...
    MPI_Datatype DataType = (sizeof(float) == 4) ? MPI_FLOAT : MPI_DOUBLE;
    MPI_Arg one = 1;
    MPI_Allreduce(&min, &gmin, one, DataType, MPI_MIN, EnzoTopComm);
  }
#else
  gmin = min;
#endif
