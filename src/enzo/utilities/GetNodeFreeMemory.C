...
int GetNodeFreeMemory(void)
{
  ...
  err = MPI_Comm_size(EnzoTopComm, &nt);
    if( err != 0 ){my_exit(EXIT_FAILURE);}
  err = MPI_Comm_rank(EnzoTopComm, &id);
    if( err != 0 ){my_exit(EXIT_FAILURE);}
...
//  necessarily the same as in the target job
  ...
  MPI_Allreduce(node_input, actual_node, nn, DataTypeInt, MPI_SUM, EnzoTopComm);

  MPI_Allreduce(free_input, actual_free, nn, DataTypeInt, MPI_SUM, EnzoTopComm);
